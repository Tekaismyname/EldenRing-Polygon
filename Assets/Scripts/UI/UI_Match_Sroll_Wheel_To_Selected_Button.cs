...
namespace TK
{
    public class UI_Match_Sroll_Wheel_To_Selected_Button : MonoBehaviour
    {
        ...
        private void SnapTo(RectTransform target)
        {
            ...
            Vector2 newPosition =(Vector2)scrollRect.transform.InverseTransformPoint(contentPane.position) 
                                    - (Vector2)scrollRect.transform.InverseTransformPoint(target.position);

            // WE ONLY WANT TO LOCK THE POSITION ON THE Y AXIS (UP AND DOWN)
            newPosition.x = 0;

            contentPane.anchoredPosition= newPosition;
        }
    }
